 ...
 * @param amount Amount of INJ to undelegate as a string (e.g., "1.5" for 1.5 INJ)
...
export async function undelegateINJ(
  ...
  console.log(
    `Undelegating ${amount} INJ from ${validatorAddress} on ${network}...`,
  );
  const walletClient = agent.getWalletClient(network);
  const publicClient = agent.getPublicClient(network);

  if (Number(amount) <= 0) {
    const errorMsg = "INJ amount to undelegate must be greater than 0";
  ...
  try {
    ...
    const delegationInfo = await agent.getDelegation(network, validatorAddress);
    if (!delegationInfo) {
    ...
    if (!hash) {
      throw new Error("Undelegate transaction failed to send");
    }

    return `Undelegated ${amount} INJ from ${validatorAddress}.\nTransaction hash for the undelegate: ${hash}`;
  } catch (error) {
